...
public class GameHandler : MonoBehaviour
{
    ...
    {
        //Debug.Log("Current inv pos " + SessionManager.Instance.CurrentInvPosition + " inv count " + SessionManager.Instance.Inventory.Count);
        if((SessionManager.Instance.CurrentInvPosition < SessionManager.Instance.Inventory.Count) && SessionManager.Instance.CurrentInvPosition > -1)
        {
            uiManager.RemoveItem(SessionManager.Instance.CurrentInvPosition);
            SessionManager.Instance.DropItem();
            uiManager.SetCurrentItem();
        }
    ...
    private void Update()
    {
        ...
        else
        {
            for (int i = 49; i < 58; i++)
                ...
                {
                    Debug.Log("Key with num " + (i - 49) + " was pressed");
